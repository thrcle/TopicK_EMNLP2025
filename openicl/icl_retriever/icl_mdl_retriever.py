...
class MDLRetriever(TopkRetriever):
    ...
    def topk_search(self):
        ...
        for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
            ...
            near_ids = self.index.search(embed, min(self.candidate_num, len(self.index_ds)))[1][0].tolist() # top-k
            candidates = []
            mdl_scores = []
    ...
    def cal_ce(self, input_texts: List[str], mask_length=None):
        ...
        # inputs = self.tokenizer(input_texts, padding=True, return_tensors='pt', truncation=True, max_length=500)
        ...
        for input_text in input_texts:
            inputs = self.tokenizer(input_text, padding=True, return_tensors='pt', truncation=True, max_length=600)
            inputs = {k: v.to(self.device) for k, v in inputs.items()}
            outputs = self.metric_model(**inputs)
