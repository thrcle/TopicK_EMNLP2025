...
class DPPRetriever(TopkRetriever):
    ...
    def dpp_search(self):
        ...
        for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
            ...
            near_ids = np.array(self.index.search(embed, self.candidate_num)[1][0].tolist()) # top-k
            ...
            samples_ids = samples_ids[(-samples_scores).argsort()].tolist() # [::-1] # descending order
    ...
    def get_kernel(self, embed, candidates):
        ...
        rel_scores = (rel_scores + 1) / 2 # [-1,1] to [0,1]

        # to prevent overflow error
        rel_scores -= rel_scores.max() # [0,1] to [-1,0]

        # to balance relevance and diversity
        rel_scores = np.exp(rel_scores / (2 * self.scale_factor)) # [-1,0] to [-5,0] to [e^-5,1]

        # to make kernel-matrix non-negative
        sim_matrix = np.matmul(near_reps, near_reps.T)
        sim_matrix = (sim_matrix + 1) / 2 # [-1,1] to [0,1]
