...
class ConERetriever(TopkRetriever):
    ...
    def topk_search(self):
        ...
        for entry in tqdm.tqdm(res_list, disable=not self.is_main_process):
            ...
            near_ids = self.index.search(embed, min(self.candidate_num, len(self.index_ds)))[1][0].tolist() # top-k (30)
            candidates = []
            mdl_scores = []
            ...
            for j in range(self.candidate_num):
                rand_idx_list = [near_ids[j]] # [j]
                candidates.append(rand_idx_list)

                # ice is the in-context demonstrations
                ice = self.generate_ice(rand_idx_list, ice_template=self.ice_template) # p(q|d_j)
                ...
                prompt = self.generate_label_prompt(idx, ice, labels[0], self.ice_template, self.prompt_template) ## label is dummy
            ...
            for batch_id in range(self.candidate_num // self.ppl_batch_size): # p(q|d_j)
                with torch.no_grad():
                    loss_list = self.cal_ce(prompts[batch_id * self.ppl_batch_size: (batch_id + 1) * self.ppl_batch_size],
                                            mask_lengths=mask_lengths[batch_id * self.ppl_batch_size: (batch_id + 1) * self.ppl_batch_size],
                                            test_lengths=test_lengths[batch_id * self.ppl_batch_size: (batch_id + 1) * self.ppl_batch_size])
            ...
            if self.candidate_num % self.ppl_batch_size != 0:
                with torch.no_grad():
                    ...
                    loss_list = self.cal_ce(prompts[end_pos:],
                                            mask_lengths=mask_lengths[end_pos:],
                                            test_lengths=test_lengths[end_pos:])
                    mdl_scores.extend(loss_list)
    ...
    def cal_ce(self, input_texts: List[str], mask_lengths=None, test_lengths=None):
        if self.metric_model is None:
            logger.info(f'Load model {self.ce_model_name} for calculating ConE...')
